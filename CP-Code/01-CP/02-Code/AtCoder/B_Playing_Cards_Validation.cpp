...
struct Answer{
    ...
    void Solve(int tc){
        ...
        /*
        int n; cin >> n;
        vector <char> firstChar = {'H', 'D', 'C', 'S'};
        vector <char> secondChar = {'A', '2', '3', '4', '5', '6', '7', '8', '9', 'T', 'J', 'Q', 'K'};
        int okay = 1;
        map <string, int> cnt;
        while (n--){
            string s; cin >> s;
        ...
        // for (auto &it : cnt){
        //     if (it.second >= 2) {okay = 0; break;}
        // }
        cout << ("Yes") << nln;
        */
        int n; cin >> n;
        int x = n;
        string a = "HCDS", b = "A23456789TJQK";
        set<string> cnt;
        while (n--){
            string s; cin >> s;
            cnt.insert(s);
            ...
            for (auto &it : a){
                if (it == s[0]) okay1 = true;
            }
            for (auto &it : b){
                if (it == s[1]) okay2 = true;
            }
            if (okay1 && okay2) continue;
            cout << "No" << nln; return;
        }
        cout << ((cnt.size() == x)? "Yes" : "No") << nln;
    }
};
/*/--------------------------------------------------------------------------/*/
