...
struct Answer{
    ...
    void Solve(int tc){
        ...
        while (q--){
            ...
            if (type == 1) follows[a].insert(b);
            else if (type == 2) follows[a].erase(b);
            else {
                // trace(follows.count({a, b}), follows.count({b, a}));
                // trace("Hey 3!");
                cout << ((follows[a].count(b) && follows[b].count(a)? "Yes" : "No")) << nln;
            }
            // trace(follows);
            /*
            Test Case 1:
