...
struct Answer{
    ...
    void Solve(int tc){
        ...
        int n; cin >> n;
        vector <ll> a(n + 1), inv_prefix(n), prefix_sum(n);
        ...
        //         inv_prefix[i] = (prefix_sum[i] - a[i - 1]);
        //         // else inv_prefix[i] = (a[i] - inv_prefix[i - 1]);
        //         // trace(inv_prefix[i]);
        //     }
        //     // cout << inv_prefix << nln;
        //     trace(inv_prefix);
        //     trace(prefix_sum); //! prefix_sum : {3, 7, 15}
        // };
        /*
            ans[1] = a[1]
            ans[2] = a[1] + a[2] = a[1]
            ans[3] = a[1] + a[2] + a[3] = ans[2] - a[1]
            ans[k] = a[k] - prefix_sum[1] to prefix_sum[k]

        */
        vector <ll> ans;
        auto inv_prefix_sum = [&](){
            for (int i = 1; i <= n; i++) ans.pb(a[i] - a[i - 1]);
            // trace(ans);
