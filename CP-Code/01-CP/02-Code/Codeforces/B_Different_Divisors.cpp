...
struct Answer{
    ...
    void Solve(int tc, vector <int> &primes){
        ...
        int n; cin >> n;
        int a = 1;
        int b = lower_bound(a + n);
        int c = lower_bound(b + n);
        // trace(a, b, c);
        cout << (ll)(a * b * c) << nln;
    }
};
...
void ShAiDSk_Solve(){
    Help H;
    int tc = 1; cin >> tc;
    vector <int> primes = H.sieve(100000);
    // trace(primes);
    // while (tc--){Answer a; a.Solve();}
    for (int i = 1; i <= tc; i++){Answer a; a.Solve(i, primes);}
}
/*/ MainFunction() /*/
