...
struct Answer{
    ...
    void Solve(int tc){
        ...
        /*
            !Author: ShAiDSk
            ?Problem_name: A. Temporarily unavailable
            *Location: https://codeforces.com/contest/1282/problem/A
        */ 
        /*/-------------------------------------------------------/*/
        int a, b, c, r; cin >> a >> b >> c >> r;
        int left = c - r, right = c + r;
        // trace(left, right);
        int start = max(left, min(a, b));
        int end = min(right, max(a, b));
        // trace(start, end);
        ll ans = abs(b - a) - max(0, end - start);
        cout << ans << nln;
    }
};
/*/--------------------------------------------------------------------------/*/
