...
struct Answer{
    ...
    void Solve(int tc){
        ...
        auto finding_good_number = [&](ll n) -> string {
            string res = "";
            while (n > 0){
                ll x = n % 3;
                n /= 3;
            ...
            // trace(res);
            res = '0' + res;
            int cnt1 = 0, cnt2 = 0, indx = 0;
            for (int i = res.length(); i > 0; i--){
                if (res[i] == '0' && !cnt1) cnt1 = 1, indx = i;
                if (res[i] == '2') cnt1 = 0, cnt2 = 1;
            }
            if (cnt1){
                if (cnt2){
                    res[indx] = '1';
                    for (int i = indx + 1; i < res.length(); i++){
                        res[i] = '0';
                    }
                }
                return res;
        ...
        ll n; cin >> n;
        // string s = finding_good_number(n);
        // trace(s);
        // reverse(all(s));
        // ll ans = 0;
        // for (int i = 0; i < s.length(); i++){
        //     ans += ll(pow(3, i))*(s[i] - 48);
        //     // trace(ans);
        // }
        // cout << ans << nln;
