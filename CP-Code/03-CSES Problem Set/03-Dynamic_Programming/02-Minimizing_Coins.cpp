...
/*/------------------------------Global-Defines-------------------------------/*/
const int mxN = 1e6, mxX = 1e6, MODi = 1e9 + 7;
// int dp[mxN + 1];
vector<ll> dp(mxN + 1, IntMax); // 1D dp initialization
// vector<vector<ll>> dp(mxN + 1, vector<ll> (mxN, 0)); // 2D dp initialization
/*/--------------------------------------------------------------------------/*/
struct Answer{
    ...
    // for Recursive dp
    int MinimizingCoins(int k, int n, vector <int> coins){
        if (k == 0) return 0;
        if (dp[k] != -1) return dp[k];
        for (int i = 0; i < n; i++){
            dp[k] = min(dp[i], dp[k - coins[i]] + 1);
        }
        return dp[k];
    }
    // 0-1 Knapsack.
    ...
    void Solve(){
        ...
        // `{`:∀:x:∀:`}`
        // State : dp[k] = minimum number of coins to make sum = k;
        // Transition: dp[i] = min(sum(dp[n - coins[i]])) form 1 to n;
        int n, k; cin >> n >> k;
        vector <ll> a(n);
